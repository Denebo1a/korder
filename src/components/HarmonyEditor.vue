      ...
      >
        <div class="row-container">
          ...
          <div 
            ...
            :class="{ 'measure-first': colIndex === 1, 'measure-last': colIndex === 4 || (rowIndex - 1) * 4 + colIndex === segment.measures }"
          >
            <!-- 小节数字标记 -->
            ...
            <div class="measure-content">
              ...
              <div class="beat-grid">
                <div 
                  v-for="beatIndex in beatsPerMeasure"
                  :key="beatIndex"
                  class="beat-cell"
                  :class="{ 
                    'beat-strong': beatIndex === 1,
                    'beat-current': isCurrentBeat((rowIndex - 1) * 4 + colIndex - 1, beatIndex - 1)
                  }"
                >
                  {{ beatIndex }}
                </div>
              ...
              <div class="harmony-layer">
                <div 
                  v-for="harmony in getMeasureHarmonies((rowIndex - 1) * 4 + colIndex - 1)"
                  :key="harmony.id"
                  class="harmony-segment"
                  :style="getHarmonyStyle(harmony, (rowIndex - 1) * 4 + colIndex - 1)"
                  @click="editHarmony(harmony)"
                >
                  <span class="harmony-chord">
                    <div class="chord-degree" v-html="getHarmonyDisplayText(harmony)"></div>
                    <div class="chord-name" v-html="getChordName(harmony)"></div>
                  </span>
                </div>
...
// 计算属性
const beatsPerMeasure = computed(() => store.timeSignature.numerator)
...
const currentBeatInSegment = computed(() => {
  ...
  return (beatInSegment >= 0 && beatInSegment < segmentTotalBeats) ? beatInSegment : -1
})
...
const isPlayingInRow = (rowIndex: number) => {
  ...
  const rowEndBeat = Math.min((rowIndex + 1) * 4 * beatsPerMeasure.value, props.segment.measures * beatsPerMeasure.value)
  
  return currentBeatInSegment.value >= rowStartBeat && currentBeatInSegment.value < rowEndBeat
}
...
const isPlayingInMeasure = (measureIndex: number) => {
  ...
  return currentBeatInSegment.value >= measureStartBeat && currentBeatInSegment.value < measureEndBeat
}
...
const getChordName = computed(() => {
  ...
  return (harmony: HarmonySegment) => {
    ...
    let chord = generateChordName(harmony.rootDegree, harmony.accidental, harmony.chordType, segmentKey) || harmony.chord || 'C'
    ...
    if (harmony.bassDegree) {
      const bassNote = generateChordName(harmony.bassDegree, harmony.bassAccidental, 'major', segmentKey)
      chord += `/${bassNote}`
    }
...
const canAddHarmonyAt = (measureIndex: number) => {
  ...
  // 检查开头是否有空间
  if (sortedHarmonies.length > 0 && sortedHarmonies[0] && sortedHarmonies[0].start > measureStart) {
    return true
  }
...
const getAddButtonStyle = (measureIndex: number) => {
  const nextBeat = store.getNextAvailableBeat(props.segment.id, measureIndex)
  const measureStart = measureIndex * beatsPerMeasure.value
  const relativePosition = nextBeat - measureStart
  const left = (relativePosition / beatsPerMeasure.value) * 100
  
  return {
    left: `${Math.min(left, 90)}%` // 确保按钮不会超出边界
  }
}
...
const addHarmony = (measureIndex: number) => {
  ...
  // 根据片段调性生成默认和弦名称
  const defaultChordName = generateChordName('I', '', 'major', segmentKey) || 'C'
