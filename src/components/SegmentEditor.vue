<template>
  <div class="segment-editor">
    <el-card
      class="segment-card"
      :class="{ 'is-collapsed': !segment.isExpanded }"
    >
      <!-- 片段头部 -->
      <template #header>
      ...
      <el-form :model="editForm" label-width="80px" :rules="formRules">
        ...
        <el-form-item label="拍号">
          ...
          <div style="font-size: 12px; color: #909399; margin-top: 4px">
            留空则使用全局拍号 ({{ store.timeSignature.numerator }}/{{
...
const formRules = {
  ...
  key: [{ required: true, message: "请选择主音", trigger: "change" }],
  mode: [{ required: true, message: "请选择调式", trigger: "change" }],
};
...
const editSegmentInfo = () => {
  editForm.value = {
    ...
    timeSignature: {
      ...
      denominator:
        props.segment.timeSignature?.denominator ||
        store.timeSignature.denominator,
    },
  };
...
const saveSegmentInfo = async () => {
  ...
  try {
    ...
    if (newMeasures < oldMeasures) {
      ...
      const conflictHarmonies = props.segment.harmonies.filter(
        (h) => h.startBeat >= maxBeat
      );
    ...
    const updatedSegment = {
      ...
      timeSignature:
        editForm.value.timeSignature.numerator ===
          store.timeSignature.numerator &&
        editForm.value.timeSignature.denominator ===
          store.timeSignature.denominator
          ? undefined
